...
use super::client::client_sync_players;
...
fn update_animation_movement(
    mut player_query: Query<(&FacingDirection, &mut Sprite, &mut PlayerAnimation)>,
) {
    for (direction, mut sprite, mut animation) in &mut player_query {
        let dx = direction.0.x;
        if dx != 0.0 {
            sprite.flip_x = dx < 0.0;
        }

        let animation_state = if direction.0 == Vec2::ZERO {
...
/// Update the sprite direction and animation state (idling/walking).
fn update_projectile_direction(
    mut projectile_query: Query<(&FacingDirection, &mut Transform), Without<PlayerAnimation>>,
) {
    for (direction, mut transform) in &mut projectile_query {
        let dx = direction.0.x;
        let dy = direction.0.y;
